...
const KYWashSystem = () => {
  ...
  const mergeMachineSnapshot = (
    ...
    return {
      ...
      timeLeft: status === 'running'
        ...
        : 0,
      mode: incomingMachine.mode ?? prevMachine?.mode ?? null,
      locked: incomingMachine.locked,
      userStudentId: incomingMachine.userStudentId ?? prevMachine?.userStudentId ?? null,
  ...
  const cancelMachineByOtherUser = (machineId: number, machineType: 'washer' | 'dryer'): void => {
    ...
    // Verification: Ask user to confirm they want to cancel an empty washer
    const isConfirmed = window.confirm(
      `Are you sure you want to cancel the ${machineType} #${machineId} because the washer is empty? This action will allow other users to use the machine again.`
    );
    ...
    // Emit to real-time API
    if (socketRef.current?.emit) {
      socketRef.current.emit('machine-cancel-by-other', {
        machineId: String(machineId),
        machineType: machineType,
        studentId: user.studentId,
      });
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {machines.filter((m: Machine) => m.type === 'washer').map((machine: Machine) => (
                      <div
                        ...
                        className={`p-4 rounded-lg border-2 transition cursor-pointer relative ${
                          machine.locked
